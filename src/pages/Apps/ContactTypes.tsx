...
const Layouts = () => {
    ...
    const getProduct = async () => {
        try {
            const res = await axios.get('http://localhost:3004/question/list6');
            console.log('ui', res);
            setData(res.data);
            setFilter(res.data);
        } catch (err) {
    ...
    const handlesubmit = (event: any) => {
        axios
            .post('http://localhost:3004/question6', values1)
            .then((res) => {
    ...
    const handlesubmitedit = (e: any) => {
        axios
            .post(`http://localhost:3004/edit/question6?id=${id}`, values1)
            .then((res) => {
        ...
        <div>
            <div className="pt-5 ">
                <div className="panel" id="registration_form">
                    <div className="mb-10">
                        <h1 className="text-3xl font-extrabold !leading-snug text-primary md:text-4xl">Contact Type</h1>
                    </div>
                    {questionInput ? (
                        <>
                            <div className="mb-5">
                                <form className="space-y-5" onSubmit={handlesubmit}>
                                    <div>
                                        <label htmlFor="ctnTextarea">Add Contact Type</label>
                                        <input type="text" placeholder="contact type" name="modes" value={values1.modes} onChange={handlecommunication} className="form-input sm:w-1/2" required />
                                    </div>
                        ...
                        <>
                            <div className="mb-5">
                                <form className="space-y-5" onSubmit={handlesubmitedit}>
                                    <div>
                                        <label htmlFor="ctnTextarea">Edit Contact type</label>
                                        <input
                                            type="text"
            ...
            <h1 className="text-lg font-semibold  p-4 pl-0">Contact Type List</h1>
            <DataTable
